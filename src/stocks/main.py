...
@click.option("--sma-200-cross", type=click.Choice(["ABOVE", "BELOW"], case_sensitive=False), default=None, help="Filter by SMA 200 crossover direction.")
@click.option("--ha-dir", type=click.Choice(["UP", "DOWN"], case_sensitive=False), default=None, help="Filter by Heikin-Ashi candle direction.")
@click.option("--renko-dir", type=click.Choice(["UP", "DOWN"], case_sensitive=False), default=None, help="Filter by latest Renko brick direction.")
@click.option("--lb-dir", type=click.Choice(["UP", "DOWN"], case_sensitive=False), default=None, help="Filter by latest Line Break line direction.")
@click.option("--limit", default=50, help="Maximum number of symbols to output.")
def screen(config_path: str, min_rsi: float, max_rsi: float, sma_200_cross: str, ha_dir: str, renko_dir: str, lb_dir: str, limit: int):
    ...
    try:
        ...
        click.echo("\n" + "="*115)
        click.echo(f"{'Symbol':<12} | {'Close':<10} | {'Change %':<10} | {'RSI 14':<8} | {'HA Candle':<9} | {'SMA 200':<8} | {'Renko':<7} | {'Line Break':<10}")
        click.echo("="*115)
...
@cli.command()
@click.option("--config-path", default="config/config.yaml", help="Path to config.yaml configuration file.")
@click.option("--symbols", default=None, help="Comma-separated list of specific symbols to recalculate (e.g. RELIANCE,TCS).")
@click.option("--limit", type=int, default=None, help="Limit the number of symbols to process (useful for testing).")
def recalculate_derived(config_path: str, symbols: str, limit: Optional[int]):
    ...
    try:
        ...
        if symbols:
            ...
            target_symbols_ns = [f"{s}.NS" if not s.endswith(".NS") and not s.startswith("^") else s for s in target_symbols]
