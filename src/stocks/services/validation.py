    ...
    def validate_prices(
        ...
        for i, row in enumerate(sorted_prices):
            ...
            try:
                ...
                if v <= 0:
                    logger.warning(
                        f"[{ticker}] Row on {t_date} skipped: Volume is zero or negative (market holiday or halt)."
                    )
                    continue
                ...
                if not (l <= o <= h):
                    logger.warning(
                        f"[{ticker}] Row on {t_date} skipped: Logical inconsistency (Open {o} out of [Low {l}, High {h}] range)."
                    )
                    continue
                if not (l <= c <= h):
                    logger.warning(
                        f"[{ticker}] Row on {t_date} skipped: Logical inconsistency (Close {c} out of [Low {l}, High {h}] range)."
                    )
                    continue
                if l > h:
        ...
        logger.info(
            f"[{ticker}] Validation complete. {len(validated_prices)}/{len(prices)} rows passed validation checks."
        )
