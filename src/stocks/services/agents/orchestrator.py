...
class AgentPool:
    ...
    def get_agent(self, agent_name: str) -> Agent:
        ...
        options = {}
        if agent_name in ["orchestrator", "report_agent", "stock_analysis_agent", "trade_planner_agent", "market_regime_agent", "opportunity_scanner_agent"]:
...
class Orchestrator:
    ...
    async def _step_database_dynamic(self, msg: str, ctx: WorkflowContext[Dict[str, Any], Dict[str, Any]]) -> None:
        ...
        from stocks.db.models import DailyHeikinAshi, RenkoBrick, LineBreakLine
        ha = self.db.scalar(select(DailyHeikinAshi).filter_by(symbol_id=sym_obj.id).order_by(DailyHeikinAshi.trading_date.desc()).limit(1))
        brick = self.db.scalar(select(RenkoBrick).filter_by(symbol_id=sym_obj.id).order_by(RenkoBrick.brick_index.desc()).limit(1))
        lb = self.db.scalar(select(LineBreakLine).filter_by(symbol_id=sym_obj.id).order_by(LineBreakLine.line_index.desc()).limit(1))
        ...
        out_msg = {
            ...
            "ha_direction": ha.ha_direction if ha and hasattr(ha, "ha_direction") else ("UP" if ha and float(ha.close) >= float(ha.open) else "DOWN"),
    ...
    async def _step_stock_analysis_dynamic(self, msg: Dict[str, Any], ctx: WorkflowContext[Dict[str, Any], Dict[str, Any]]) -> None:
    ...
    async def _step_trade_planner_dynamic(self, msg: Dict[str, Any], ctx: WorkflowContext[Dict[str, Any], Dict[str, Any]]) -> None:
    ...
    async def _step_market_regime_breakout(self, msg: str, ctx: WorkflowContext[Dict[str, Any], Dict[str, Any]]) -> None:
        ...
        if nifty_sym:
            ...
            nifty_ind = self.db.scalar(
                select(DailyIndicator).filter_by(symbol_id=nifty_sym.id).order_by(DailyIndicator.trading_date.desc()).limit(1)
            )
            if nifty_ind:
    ...
    async def _step_screening_swing(self, msg: Dict[str, Any], ctx: WorkflowContext[Dict[str, Any], Dict[str, Any]]) -> None:
    ...
    async def _step_stock_analysis_swing(self, msg: Dict[str, Any], ctx: WorkflowContext[Dict[str, Any], Dict[str, Any]]) -> None:
        ...
        for cand in candidates:
            ...
            prices = self.db_service.get_prices_for_window(sym_obj.id, datetime.date.today() - datetime.timedelta(days=100))
    ...
    async def _step_trade_planner_swing(self, msg: Dict[str, Any], ctx: WorkflowContext[str, str]) -> None:
        ...
        for cand in candidates:
            ...
            indicator = self.db.scalar(
                select(DailyIndicator).filter_by(symbol_id=cand["sym_obj_id"]).order_by(DailyIndicator.trading_date.desc()).limit(1)
            )
    ...
    async def execute_workflow(self, user_query: str) -> AsyncGenerator[str, None]:
        ...
        if user_query_clean.startswith("STOCKS:") or user_query_clean.startswith("SELECT") or "SQL" in user_query.upper():
            is_sql_query = True

        if is_sql_query:
            yield self._sse_event("started", "Activating SQL Ingestion & Screening Agent...")
            yield self._sse_event("intent_detected", {
                "intent": "sql_screening",
                "symbol": None,
                "rationale": "Direct SQL Filtering Request detected."
            })
            ...
            for attempt in range(1, max_retries + 1):
                ...
                try:
                    ...
                    yield self._sse_event("agent_active", {
                        "agent": "sql_data_agent",
                        "status": f"Verifying generated SQL SELECT query..."
                    })
                    ...
                    yield self._sse_event("agent_active", {
                        ...
                        "status": "Running SQL query on MS SQL Server..."
                    })
            ...
            if sql_error:
                # All attempts failed
                yield self._sse_event("error", f"SQL screening failed after {max_retries} self-healing attempts. Error: {sql_error}")
                return
            ...
            async for event in stream:
                if event.type == "executor_invoked":
                    yield self._sse_event("agent_active", {
                        ...
                        "status": f"Activating {event.executor_id.replace('_', ' ')}..."
                    })
