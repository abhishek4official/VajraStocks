  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-1">
        ...
        <span className={`shrink-0 text-[10px] ${isActive ? 'text-purple-200' : 'text-slate-600'}`}>{label}</span>
      </div>
      <div className={`text-[10px] mt-0.5 ${isActive ? 'text-purple-200' : 'text-slate-600'}`}>
      ...
      <div className="p-3 border-b border-slate-800">
        ...
        {AGENT_TYPES.map((agent) => (
          <button
            ...
            className={`w-full flex items-center gap-2 px-2.5 py-2 rounded-lg text-xs font-medium mb-1 transition-colors ${
