    ...
    <div className="w-full flex flex-col">
      <div className="flex items-center justify-between mb-3 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-white tracking-wide">NSE Announcements</h3>
          {items.length > 0 && (
      ...
      <div className="flex-1 overflow-y-auto flex flex-col gap-2 max-h-96 pr-1">
        {loading ? (
          <div className="text-xs text-slate-500 py-4 text-center animate-pulse">Loading announcements…</div>
        ) : items.length === 0 ? (
          ...
          items.map(item => (
            <div
              key={item.id}
              className="rounded-lg border border-slate-800 bg-slate-900/40 hover:bg-slate-900/70 transition p-2.5"
            >
              <div className="flex items-start justify-between gap-2">
                ...
                >
                  <p className="text-xs font-semibold text-slate-200 leading-snug line-clamp-2">{item.subject}</p>
                </button>
                <div className="flex items-center gap-1.5 shrink-0 mt-0.5">
                  <span className="text-[10px] text-slate-500 font-mono whitespace-nowrap">{timeAgo(item.announcement_date)}</span>
                  {item.file_url && (
                    <a
                      href={item.file_url}
                      target="_blank"
              ...
              {expanded === item.id && item.description && (
                <p className="mt-2 text-[11px] text-slate-400 leading-relaxed border-t border-slate-800 pt-2">
