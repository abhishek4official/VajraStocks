...
import {
  ...
  Clock, Cpu, History, Play, RefreshCw, StopCircle, Zap,
} from 'lucide-react';
import { API_BASE } from '../lib/apiBase';
import type { MLFoldMetric, MLProgressEvent, MLTrainingRun } from '../services/api';
...
export const MLTrainingPanel: React.FC = () => {
  ...
  const [foldMetrics,  setFoldMetrics]  = useState<MLFoldMetric[]>([]);
  ...
  const [latestRun,    setLatestRun]    = useState<MLTrainingRun | null>(null);
  const [history,      setHistory]      = useState<MLTrainingRun[]>([]);
  ...
  const fetchLatest = useCallback(async () => {
    try {
      const r = await fetch(`${BASE}/ml/latest`);
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const r = await fetch(`${BASE}/ml/history`);
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const r = await fetch(`${BASE}/ml/train/status`);
      if (!r.ok) return;
      const d = await r.json();
      if (d.status === 'running' && !isTraining) {
        setIsTraining(true);
        openStream();
      }
    } catch { /* silent */ }
  ...
  const openStream = useCallback(() => {
    ...
    const es = new EventSource(`${BASE}/ml/train/stream`);
    esRef.current = es;

    es.onmessage = (e: MessageEvent) => {
      let evt: MLProgressEvent;
      ...
      switch (evt.type) {
        ...
        case 'device':
          setDevice(evt.device ?? null);
          addLog(`Hardware: ${evt.device?.toUpperCase() ?? 'unknown'} selected`);
          break;
        ...
        case 'dataset':
          ...
          addLog(`Dataset: ${(evt.rows ?? 0).toLocaleString()} rows × ${evt.features} features (${evt.date_start} → ${evt.date_end})`);
          break;
        ...
        case 'fold_start':
          ...
          setStage(`Fold ${evt.fold}/${evt.total}: LightGBM training...`);
        ...
        case 'fold_done':
          setFoldMetrics(prev => {
            ...
            return [...next, {
              fold:     evt.fold     ?? 0,
              lgbm_ic:  evt.lgbm_ic  ?? 0,
              ridge_ic: evt.ridge_ic ?? 0,
              hit_rate: evt.hit_rate ?? 0,
              ls_pnl:   evt.ls_pnl   ?? 0,
            }].sort((a, b) => a.fold - b.fold);
          });
          addLog(`Fold ${evt.fold} done — IC ${(evt.lgbm_ic ?? 0) >= 0 ? '+' : ''}${(evt.lgbm_ic ?? 0).toFixed(4)}  Hit ${(evt.hit_rate ?? 0).toFixed(1)}%  L/S ${(evt.ls_pnl ?? 0) >= 0 ? '+' : ''}${(evt.ls_pnl ?? 0).toFixed(2)}%`);
          break;
        ...
        case 'complete':
          ...
          addLog(`Training complete — Mean IC ${(evt.mean_ic ?? 0) >= 0 ? '+' : ''}${(evt.mean_ic ?? 0).toFixed(4)} across ${evt.folds} folds`);
          fetchLatest();
          fetchHistory();
          es.close();
          break;
  ...
  const handleTrain = async () => {
    ...
    try {
      const r = await fetch(`${BASE}/ml/train`, { method: 'POST' });
      if (!r.ok) {
      ...
      const d = await r.json();
      setIsTraining(true);
      addLog(`Training job started — version ${d.version} (run #${d.run_id})`);
      openStream();
    } catch (e: unknown) {
  ...
  const handleCancel = async () => {
    ...
    try {
      await fetch(`${BASE}/ml/train`, { method: 'DELETE' });
  ...
  // ── Derived ─────────────────────────────────────────────────────────────────
  const meanIc = foldMetrics.length
    ? foldMetrics.reduce((s, f) => s + f.lgbm_ic, 0) / foldMetrics.length
    : null;
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-white">VajraML Training</h2>
            <p className="text-xs text-slate-500">Walk-forward LightGBM — 6-fold validation</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {isTraining ? (
            ...
            <button
              onClick={handleTrain}
              className="flex items-center gap-2 px-5 py-2 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-sm font-bold transition shadow-lg shadow-purple-900/30 cursor-pointer"
            >
      ...
      {(isTraining || pct > 0) && (
        <div className="rounded-xl border border-purple-500/30 bg-[#0f101a] p-5 space-y-4">
          {/* Stage + device */}
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            {device && (
              <span className={`flex items-center gap-1.5 px-2 py-0.5 rounded border text-xs font-bold ${
                device === 'gpu'
                  ? 'bg-amber-500/15 border-amber-500/40 text-amber-300'
                  : 'bg-slate-700/40 border-slate-600/40 text-slate-400'
              }`}>
                {device === 'gpu' ? <Zap className="w-3 h-3" /> : <Cpu className="w-3 h-3" />}
                {device.toUpperCase()}
              </span>
            )}
          </div>
              ...
              <table className="w-full text-xs border-collapse">
                <thead>
                  <tr className="text-slate-500 border-b border-slate-800">
                    ...
                    <th className="py-1 px-2 text-right">LGBM IC</th>
                    <th className="py-1 px-2 text-right">Ridge IC</th>
                    <th className="py-1 px-2 text-right">Hit %</th>
                    <th className="py-1 px-2 text-right">L/S %</th>
                  </tr>
                </thead>
                <tbody>
                  {foldMetrics.map(f => (
                    <tr key={f.fold} className="border-b border-slate-800/50">
                      ...
                      <td className={`py-1 px-2 text-right font-mono font-bold ${icColor(f.lgbm_ic)}`}>
                        {f.lgbm_ic >= 0 ? '+' : ''}{f.lgbm_ic.toFixed(4)}
                      </td>
                      <td className={`py-1 px-2 text-right font-mono ${icColor(f.ridge_ic)}`}>
                        {f.ridge_ic >= 0 ? '+' : ''}{f.ridge_ic.toFixed(4)}
                      </td>
                      <td className="py-1 px-2 text-right font-mono text-slate-300">{f.hit_rate.toFixed(1)}%</td>
                      <td className={`py-1 px-2 text-right font-mono ${f.ls_pnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {f.ls_pnl >= 0 ? '+' : ''}{f.ls_pnl.toFixed(2)}%
                      </td>
                    </tr>
                  ))}
                  {foldMetrics.length > 1 && (
                    <tr className="border-t border-slate-700 bg-slate-900/40">
                      <td className="py-1 px-2 font-mono text-slate-500">Mean</td>
                      <td className={`py-1 px-2 text-right font-mono font-bold ${icColor(meanIc)}`}>
                        {meanIc != null ? `${meanIc >= 0 ? '+' : ''}${meanIc.toFixed(4)}` : '—'}
                      </td>
                      <td colSpan={3} />
                    </tr>
                  )}
      ...
      {latestRun && !isTraining && (
        <div className="rounded-xl border border-slate-800 bg-[#0c0f17] p-4">
          <div className="flex items-center gap-2 mb-3">
            <CheckCircle2 className="w-4 h-4 text-emerald-400" />
            <span className="text-sm font-bold text-white">Active Model</span>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              { label: 'Device',     value: (latestRun.device ?? '—').toUpperCase() },
              ...
              { label: 'Mean IC',    value: latestRun.mean_ic != null ? `${latestRun.mean_ic >= 0 ? '+' : ''}${latestRun.mean_ic.toFixed(4)}` : '—', highlight: true },
            ].map(({ label, value, highlight }) => (
              <div key={label}>
                <p className="text-[10px] text-slate-600 uppercase tracking-wider">{label}</p>
                <p className={`text-xs font-mono font-semibold mt-0.5 ${highlight ? icColor(latestRun.mean_ic) : 'text-slate-300'}`}>{value}</p>
              </div>
            ))}
          </div>

          {/* Per-fold metrics for last run */}
          ...
          <div className="h-48 overflow-y-auto p-3 font-mono text-[11px] text-slate-400 space-y-0.5">
            {logs.map((line, i) => (
              <div key={i} className={
                ...
                line.includes('Fold') ? 'text-purple-300' :
                'text-slate-400'
              }>{line}</div>
            ))}
            <div ref={logEndRef} />
      ...
      <div className="rounded-xl border border-slate-800 bg-[#0c0f17]">
        ...
        >
          ...
          {showHistory ? <ChevronUp className="w-3.5 h-3.5 ml-auto text-slate-600" /> : <ChevronDown className="w-3.5 h-3.5 ml-auto text-slate-600" />}
        </button>

        {showHistory && (
          <div className="border-t border-slate-800 overflow-x-auto">
            {history.length === 0 ? (
              <p className="text-xs text-slate-600 text-center py-6">No training history yet.</p>
            ) : (
              <table className="w-full text-xs border-collapse">
                <thead>
                  <tr className="text-slate-500 border-b border-slate-800 text-left">
                    ...
                    <th className="py-2 px-3 text-right">Mean IC</th>
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {history.map(run => (
                    <tr key={run.id} className="hover:bg-slate-900/40 transition">
                      <td className="py-2 px-3 font-mono text-purple-300">{run.version}</td>
                      ...
                      <td className="py-2 px-3">
                        {run.device ? (
                          <span className={`flex items-center gap-1 w-fit px-1.5 py-0.5 rounded border text-[10px] font-bold ${
                            run.device === 'gpu'
                              ? 'bg-amber-500/15 border-amber-500/40 text-amber-300'
                              : 'bg-slate-700/40 border-slate-600/40 text-slate-400'
                          }`}>
                            {run.device === 'gpu' ? <Zap className="w-2.5 h-2.5" /> : <Cpu className="w-2.5 h-2.5" />}
                            {run.device.toUpperCase()}
                          </span>
                        ) : '—'}
                      </td>
                      <td className="py-2 px-3 text-right font-mono text-slate-400">{run.num_folds ?? '—'}</td>
                      <td className={`py-2 px-3 text-right font-mono font-bold ${icColor(run.mean_ic)}`}>
                        {run.mean_ic != null ? `${run.mean_ic >= 0 ? '+' : ''}${run.mean_ic.toFixed(4)}` : '—'}
                      </td>
