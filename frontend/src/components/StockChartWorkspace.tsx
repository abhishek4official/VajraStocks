...
import { Bookmark, TrendingUp, X } from 'lucide-react';
...
export const StockChartWorkspace: React.FC<StockChartWorkspaceProps> = ({
  onClose,
    ...
    <div className="flex-1 flex flex-col gap-4">
      ...
      {activeSymbolDetail ? (
        <div className="flex flex-col gap-2 p-4 rounded-xl border border-border-subtle bg-bg-surface/35">
          {/* Row 1: Name + badges + watchlist/close */}
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <h2 className="text-sm text-slate-400 font-semibold">{activeSymbolDetail.company_name}</h2>
            </div>
            <div className="flex items-center gap-2">
              ...
              {onClose && (
                <button
                  onClick={onClose}
                  className="p-1.5 rounded-lg border border-border-subtle hover:bg-slate-800 hover:text-text-main text-slate-400 transition cursor-pointer"
                >
                  <X className="w-4 h-4" />
                </button>
              )}
              ...
              {rsScore != null && (
                <div className="flex items-center gap-1 text-xs">
                  ...
                  <span className={`font-mono font-bold px-1.5 py-0.5 rounded border text-[10px] ${
                    ...
                    : rsScore >= 0.8 ? 'text-slate-350 bg-slate-900/40 border-slate-800'
        ...
        <div className="p-4 rounded-xl border border-slate-800 bg-[#121620]/30 text-center text-slate-500 text-sm">
          Please select an active ticker to inspect market trends.
        </div>
      )}

      {/* Central Chart Canvas */}
      {activeSymbol ? (
        <div className="flex-1 min-h-[450px] relative border border-border-subtle rounded-xl overflow-hidden bg-bg-base/30">
          {isLoading && (
            <div className="absolute inset-0 flex items-center justify-center bg-bg-surface/50 backdrop-blur-xs z-30">
              <div className="flex flex-col items-center gap-2">
                <RefreshCw className="w-8 h-8 text-purple-500 animate-spin" />
                <span className="text-xs text-slate-400 font-semibold">Loading stock chart data...</span>
              </div>
            </div>
          )}
          ...
          {candles.length > 0 ? (
            <PriceChart
              ...
              onChartClick={(price) => {
                if (activeSymbol) {
                  addCustomLine(activeSymbol, price);
                  setDrawMode(false);
                }
              }}
            />
            ...
            !isLoading && (
              <div className="absolute inset-0 flex items-center justify-center text-slate-500 text-sm">
                No historical candle data synced for this symbol. Run Recalculate/Sync.
              </div>
            )
          )}
        </div>
