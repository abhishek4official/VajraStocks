...
class MarketStructureEngine:
    ...
    def generate_heikin_ashi(self, df: pd.DataFrame, prev_candle: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    def generate_renko_bricks(
        self, df: pd.DataFrame, last_brick: dict[str, Any] | None = None, pct_brick_size: float = 0.01
    ) -> list[dict[str, Any]]:
        ...
        if last_brick:
            ...
            prices_to_process = (
                df_sorted[df_sorted.index.date > last_date]
    ...
    def generate_line_breaks(
        ...
        if last_lines:
            ...
            first_line = {
                "line_index": line_idx,
                "start_date": df_sorted.index[0].date()
                if isinstance(df_sorted.index, pd.DatetimeIndex)
                else df_sorted.index[0],
                "end_date": df_sorted.index[1].date()
                if isinstance(df_sorted.index, pd.DatetimeIndex)
                else df_sorted.index[1],
                "open": close_0,
                "close": close_1,
                "direction": dir_str,
            }
