...
from stocks.db.models import DailyIndicator, Symbol
...
def get_indicators_history(symbol: str, db: Session = Depends(get_db)):
    ...
    rows = db.scalars(
        select(DailyIndicator).where(DailyIndicator.symbol_id == symbol_id).order_by(DailyIndicator.trading_date.asc())
    ).all()
...
def get_specific_indicator_history(symbol: str, indicator: str, db: Session = Depends(get_db)):
    ...
    if not hasattr(DailyIndicator, ind_name):
        raise HTTPException(
            status_code=400, detail=f"Indicator '{indicator}' is not valid. Choose from standard indicator attributes."
        )
    ...
    return [{"time": r.trading_date.strftime("%Y-%m-%d"), "value": getattr(r, ind_name)} for r in rows]
